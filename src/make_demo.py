...
column_names = ["epoch", "train loss", "train err", "Dloss", "Dscore",
                "test loss", "test err",
                "bag AUC", "bag p", "bag r", "bag f", "bag a",
                "ins AUC", "ins p", "ins r", "ins f", "ins a"]
...
def get_data_loader(dataset_name):
    ...
    if dataset_name == "bc":
        ...
        bcb.set_train()
        bcb2 = BCBags(seed=seed)
        bcb2.set_test()
        train_loader = data_utils.DataLoader(bcb, batch_size=1, shuffle=True)
        test_loader = data_utils.DataLoader(bcb2, batch_size=1, shuffle=False)
        return train_loader, test_loader
...
def train(epoch, model, optimizer, train_loader):
    ...
    info_column_value = [epoch, train_loss, train_error, d_loss, d_score]
...
def evaluate_cmd(args):
    ...
    if args.dataset == "mb":
        ...
        model = ModelFramework(encoder=Encoder_Classic1, attention_model=MultiDimAttention, L=64, K=args.dim)
    elif args.dataset in ["fox", "tiger", "elephant"]:
        model = ModelFramework(encoder=Encoder_Classic2, attention_model=MultiDimAttention, L=64, K=args.dim)
    elif args.dataset == "bc":
        model = ModelFramework(encoder=Encoder_BC, attention_model=MultiDimAttention, K=args.dim)
    else:
    ...
    return df, stopping_value, model


if __name__ == "__main__":
    multipref_path = "multi_pref"
    singlepref_path = "single_pref"
    args = parser.parse_args()
    # evaluate_cmd()
    os.environ["CUDA_VISIBLE_DEVICES"]= str(args.gpu)
    args.dim = 5
    *_, multipref_model = evaluate_cmd(args)
    args.dim = 1
    *_, singlepref_model = evaluate_cmd(args)
    ...
    for patches, label, eids, xys in zip(ccb.o_patches, ccb.o_labels, ccb.o_eids, ccb.o_xys):
        data = torch.from_numpy(patches).cuda()


        loss, error, y_prob, Y_hat, score, feature = singlepref_model.bag_eval(data, label)
        if Y_hat != label:
            continue
        inside_bag_score = (score - torch.min(score) + 1e-5) / (torch.max(score) - torch.min(score) + 1e-5)
        ins_score_single= inside_bag_score.cpu().data.numpy().reshape(-1).tolist()
        ins_preds_single = (inside_bag_score > args.inst_thr).cpu().data.numpy().reshape(-1).tolist()
        ps, rs, fs, _ = metrics.precision_recall_fscore_support(eids, ins_preds_single, average='binary')

        loss, error, y_prob, Y_hat, score, feature = multipref_model.bag_eval(data, label)
        if Y_hat != label:
            continue
        inside_bag_score = (score - torch.min(score) + 1e-5) / (torch.max(score) - torch.min(score) + 1e-5)
        ins_score_multi= inside_bag_score.cpu().data.numpy().reshape(-1).tolist()
        ins_preds_multi = (inside_bag_score > args.inst_thr).cpu().data.numpy().reshape(-1).tolist()
        pm, rm, fm, _ = metrics.precision_recall_fscore_support(eids, ins_preds_multi, average='binary')

        if rm > rs and len(eids) > 50:
